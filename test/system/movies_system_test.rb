...
class MoviesSystemTest < ApplicationSystemTestCase
  ...
  test "show lists director, producer, actor" do
    movie = create(:movie)
    create_list(:actor, 4, movies: [movie])
    
    visit movie_path(movie)
  
    assert_text movie.director_name # taking movie.director.name one step further
    assert_text movie.producer_name # taking movie.producer.name one step further
    assert_text "# of Actors in #{movie.title}: 4"
  end
